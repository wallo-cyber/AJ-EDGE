...
import { readFollowUps, writeFollowUps, type FollowUp, type FollowUpPriority, type FollowUpStatus, type FollowUpType } from '../../lib/follow-up-store';
...
export default function FollowUpsPage() {
  ...
  useEffect(() => {
    setFollowUps(readFollowUps());
    setCompanies(readCompanies());

    const handleFollowUpsUpdated = (event: Event) => {
      const customEvent = event as CustomEvent<FollowUp[]>;
      setFollowUps(customEvent.detail ?? readFollowUps());
    };

    window.addEventListener('follow-ups:updated', handleFollowUpsUpdated as EventListener);
    return () => window.removeEventListener('follow-ups:updated', handleFollowUpsUpdated as EventListener);
  }, []);
  ...
  const handleSubmit = (followUp: FollowUp) => {
    const nextFollowUps = editingFollowUpId
      ? followUps.map((item) => (item.id === editingFollowUpId ? { ...item, ...followUp } : item))
      : [{ ...followUp, id: crypto.randomUUID(), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, ...followUps];
  ...
  const handleDelete = (followUpId: string) => {
    const nextFollowUps = followUps.filter((item) => item.id !== followUpId);
    setFollowUps(nextFollowUps);
    writeFollowUps(nextFollowUps);
  };
  ...
  const handleMarkCompleted = (followUpId: string) => {
    const nextFollowUps = followUps.map((item) => item.id === followUpId ? { ...item, status: 'مكتملة' as const, updatedAt: new Date().toISOString() } : item);
    setFollowUps(nextFollowUps);
    writeFollowUps(nextFollowUps);
  };

  const handleReschedule = (followUp: FollowUp) => {
    const nextFollowUps = followUps.map((item) => item.id === followUp.id ? { ...item, status: 'مجدولة' as const, nextFollowUpDate: new Date().toISOString().slice(0, 10), updatedAt: new Date().toISOString() } : item);
    setFollowUps(nextFollowUps);
    writeFollowUps(nextFollowUps);
  };

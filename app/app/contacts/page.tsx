...
export default function ContactsPage() {
  ...
  return <CRMPage title="جهات الاتصال" description="الأشخاص المرتبطون بالشركات، مع حالة مستقلة لصانع القرار والتحقق ومصدر الدليل." action={<button onClick={() => { setEditing(null); setShowForm(true); }} className="rounded-full bg-[#2f2417] px-5 py-3 text-sm font-semibold text-white">إضافة جهة اتصال</button>}>
    ...
    {loading ? <div className="crm-empty animate-pulse">جارٍ تحميل جهات الاتصال...</div> : contacts.length === 0 ? <div className="crm-empty"><h3 className="font-bold text-[#2f2417]">لا توجد جهات اتصال موثقة بعد</h3><p className="mx-auto mt-2 max-w-2xl text-sm">لن ينشئ ALGAEU أشخاصاً أو صناع قرار دون مصدر ودليل. ابدأ من شركة محددة ثم وثّق الشخص الصحيح.</p><div className="mt-4 flex flex-wrap justify-center gap-2"><button onClick={() => { setEditing(null); setShowForm(true); }} className="btn-primary">إضافة جهة اتصال</button><a href="/companies?view=missing-dm" className="btn-secondary">الشركات التي تحتاج صانع قرار</a><a href="/research?tab=manual" className="btn-ghost">بدء بحث يدوي</a></div></div> : <>
      <div className="hidden overflow-x-auto rounded-2xl border bg-white md:block"><table className="min-w-full text-right text-sm"><thead><tr><th className="p-3">الشركة</th><th className="p-3">القسم</th><th className="p-3">الشخص</th><th className="p-3">الدور</th><th className="p-3">الهاتف</th><th className="p-3">الجوال</th><th className="p-3">البريد</th><th className="p-3">LinkedIn</th><th className="p-3">المصدر</th><th className="p-3">الثقة</th><th className="p-3">تاريخ التحقق</th><th className="p-3">الإجراءات</th></tr></thead><tbody>{contacts.map(contact => <tr key={contact.id} className="border-t"><td className="p-3">{contact.companyName || '—'}</td><td className="p-3">{contact.department || '—'}</td><td className="p-3 font-bold">{contact.fullName}</td><td className="p-3">{contact.position || '—'}</td><td className="p-3">{contact.phone || '—'}</td><td className="p-3">{contact.mobile || '—'}</td><td className="p-3">{contact.email || '—'}</td><td className="p-3">{contact.linkedIn || '—'}</td><td className="p-3">{contact.source || '—'}</td><td className="p-3">{contact.confidence || 0}%</td><td className="p-3">{contact.verifiedAt ? contact.verifiedAt.slice(0, 10) : '—'}</td><td className="p-3"><Actions contact={contact} view={setSelected} edit={value => { setEditing(value); setShowForm(true); }} archive={archive} /></td></tr>)}</tbody></table></div>
      <div className="grid gap-3 md:hidden">{contacts.map(contact => <article key={contact.id} className="crm-card p-4"><div className="flex items-start justify-between gap-2"><div><strong>{contact.fullName}</strong><p className="text-xs text-[#75664d]">{contact.companyName || 'بدون شركة'} · {contact.department || '—'} · {contact.position || '—'}</p></div><span className="crm-chip bg-amber-50">{contact.verificationStatus || 'UNVERIFIED'}</span></div><p className="mt-2 text-xs">هاتف: {contact.phone || '—'} · جوال: {contact.mobile || '—'}</p><p className="text-xs">Email: {contact.email || '—'}</p><p className="text-xs">Source: {contact.source || '—'} · {contact.confidence || 0}% · {contact.verifiedAt ? contact.verifiedAt.slice(0, 10) : 'غير موثق'}</p><p className="mt-2 text-sm">صانع قرار موثق: {contact.decisionMaker && contact.verificationStatus === 'VERIFIED' ? 'نعم' : 'لا'}</p><div className="mt-3"><Actions contact={contact} view={setSelected} edit={value => { setEditing(value); setShowForm(true); }} archive={archive} /></div></article>)}</div>
    </>}
    ...
    {selected && <section className="crm-card p-5"><div className="flex justify-between"><h3 className="font-bold">{selected.fullName}</h3><button onClick={() => setSelected(null)}>إغلاق</button></div><dl className="mt-4 grid gap-3 text-sm md:grid-cols-2"><div><dt className="text-[#75664d]">البريد</dt><dd dir="ltr">{selected.email || '—'}</dd></div><div><dt className="text-[#75664d]">الهاتف</dt><dd dir="ltr">{selected.phone || '—'}</dd></div><div><dt className="text-[#75664d]">الجوال</dt><dd dir="ltr">{selected.mobile || '—'}</dd></div><div><dt className="text-[#75664d]">LinkedIn</dt><dd>{selected.linkedIn || '—'}</dd></div><div><dt className="text-[#75664d]">المصدر</dt><dd>{selected.source || '—'}</dd></div><div><dt className="text-[#75664d]">الثقة</dt><dd>{selected.confidence || 0}%</dd></div><div><dt className="text-[#75664d]">تاريخ التحقق</dt><dd>{selected.verifiedAt ? selected.verifiedAt.slice(0, 10) : '—'}</dd></div></dl></section>}
  </CRMPage>;
}

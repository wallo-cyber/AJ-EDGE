...
const canonicalStatus = (value: unknown, hasPortal = false) => {
  const status = safe(value);
  if (['Available', 'Registration Available'].includes(status)) return 'Portal Found';
  if (['Registration Started', 'Registration Required', 'In Progress', 'Manual Action Required'].includes(status)) return 'Preparing';
  if (status === 'Not Found') return 'Not Checked';
  return statuses.includes(status) ? status : hasPortal ? 'Portal Found' : 'Not Checked';
};
...
export default function VendorRegistrationPage() {
  ...
  const visible = useMemo(() => rows.filter((row) => {
    const status = canonicalStatus(row.vendor_registration_status, Boolean(row.vendor_registration_url));

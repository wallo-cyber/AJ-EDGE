...
const statuses = ['Not Found', 'Portal Found', 'Registration Required', 'In Progress', 'Submitted', 'Approved', 'Rejected', 'Manual Action Required'];
...
export default function VendorRegistrationPage() {
  ...
  const visible = useMemo(() => rows.filter((row) => {
    const status = safe(row.vendor_registration_status) || (row.vendor_registration_url ? 'Portal Found' : 'Not Found');
  ...
  return <CRMPage title="تسجيل الموردين" description="إدارة بوابات التسجيل والإجراءات اليدوية بدون إرسال أو تسجيل خارجي تلقائي.">
    ...
    {loading ? <div className="crm-empty animate-pulse">جارٍ تحميل سجلات الموردين...</div> : <div className="grid gap-4 lg:grid-cols-2">{visible.map((row) => <article key={row.id} className="crm-card p-4"><div className="flex items-start justify-between gap-3"><div><Link href={`/companies/${row.id}`} className="font-bold hover:text-[#9a742b]">{safe(row.company_name)}</Link><p className="mt-1 text-xs text-[#75664d]">Priority {safe(row.priority) || 'C'} · {safe(row.company_type)} · {safe(row.city)}</p></div><span className="crm-chip bg-amber-50 text-amber-800">{safe(row.vendor_registration_status) || (row.vendor_registration_url ? 'Portal Found' : 'Not Found')}</span></div><div className="mt-4 grid gap-3"><input defaultValue={safe(row.vendor_registration_url)} onBlur={(event) => void save(row, { vendor_registration_url: event.target.value, vendor_registration_status: event.target.value ? 'Portal Found' : 'Not Found' })} placeholder="رابط بوابة التسجيل الرسمية" className="rounded-xl border p-2.5 text-sm"/><select defaultValue={safe(row.vendor_registration_status) || 'Not Found'} onChange={(event) => void save(row, { vendor_registration_status: event.target.value })} className="rounded-xl border p-2.5 text-sm">{statuses.map((status) => <option key={status}>{status}</option>)}</select><textarea defaultValue={safe(row.vendor_registration_requirements)} onBlur={(event) => void save(row, { vendor_registration_requirements: event.target.value })} placeholder="المتطلبات المؤكدة" className="min-h-20 rounded-xl border p-2.5 text-sm"/><input defaultValue={safe(row.vendor_registration_account_status)} onBlur={(event) => void save(row, { vendor_registration_account_status: event.target.value })} placeholder="حالة الحساب أو التسجيل" className="rounded-xl border p-2.5 text-sm"/><input defaultValue={safe(row.vendor_registration_next_action)} onBlur={(event) => void save(row, { vendor_registration_next_action: event.target.value })} placeholder="الإجراء التالي" className="rounded-xl border p-2.5 text-sm"/><textarea defaultValue={safe(row.vendor_registration_notes)} onBlur={(event) => void save(row, { vendor_registration_notes: event.target.value })} placeholder="ملاحظات" className="min-h-20 rounded-xl border p-2.5 text-sm"/></div></article>)}{!visible.length && <div className="crm-empty lg:col-span-2">لا توجد سجلات مطابقة.</div>}</div>}
  </CRMPage>;
}

...
export default function VendorsPage() {
  ...
  async function bulkDelete() {
    ...
    if (
      !confirm(
        `حذف ${targets.length} مورد نهائيًا؟ لا يمكن التراجع.\n\nالأفضل عادةً "إيقاف" الموردين بدل حذفهم.`,
      )
    )
      return;
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'تعذر حذف بعض الموردين المحددين.');
    } finally {
                ...
                <tr className="border-b border-[var(--nav-border)]">
                  <th className={`${th} w-10`}>
                    <input
                      ...
                      aria-label="تحديد كل الموردين الظاهرين"
                      className="h-4 w-4"
                    />
                  </th>
                  <th className={th}>الشركة</th>

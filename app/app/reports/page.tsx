...
export default function ReportsPage(){
 ...
 const coverage=decisionMakerCoverage(companies,contacts); const outbound=new Set(events.filter(e=>e.direction==='OUTBOUND').map(e=>e.company_id)).size, replies=new Set(events.filter(e=>e.direction==='INBOUND').map(e=>e.company_id)).size; const active=opps.filter(o=>!['WON','LOST'].includes(safe(o.stage))).length, won=opps.filter(o=>safe(o.stage)==='WON').length, lost=opps.filter(o=>safe(o.stage)==='LOST').length, vendorReady=companies.filter(c=>Boolean(c.vendor_registration_url)).length, researchBacklog=(data.agent_jobs??[]).filter(j=>j.status==='manual_research_required'&&ids.has(safe(j.company_id))).length;
 ...
 return <CRMPage title="التقارير" description="مؤشرات لاتخاذ القرار من بيانات Supabase الفعلية؛ المسودة ليست تواصلاً والرد يعتمد على حدث وارد موثق." action={<button onClick={exportCsv} className="btn-secondary">تصدير CSV</button>}>
  ...
  {loading?<div className="crm-empty animate-pulse">جارٍ إعداد مؤشرات القرار…</div>:<>
   ...
   <section className="crm-card p-4"><h3 className="section-title">مسار التحويل واتخاذ القرار</h3><div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-5">{[['مستهدفة',companies.length],['صانع قرار موثق',coverage.covered],['جاهزة للتواصل',companies.filter(c=>['DRAFT_READY','APPROVED'].includes(companyOutreachState(c,contacts,messages,events))).length],['تم التواصل',outbound],['ردود',replies],['فرص نشطة',active],['فوز',won],['خسارة',lost],['بوابات موردين',vendorReady],['بحث يدوي',researchBacklog]].map(([l,v])=><div key={String(l)} className="rounded-xl bg-[#f8f1e4] p-3 text-center"><b className="block text-xl">{v}</b><span className="text-xs text-[#75664d]">{l}</span></div>)}</div></section>

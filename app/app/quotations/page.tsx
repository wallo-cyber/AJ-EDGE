...
export default function QuotationsPage() {
  ...
  async function bulkDelete() {
    ...
    if (!window.confirm(`حذف ${selectedIds.size} عرض نهائيًا؟ لا يمكن التراجع.`)) return;
    setBulkDeleting(true);
    setError('');
    try {
      await Promise.all([...selectedIds].map((id) => quotationsApi.remove(id)));
      setQuotations((items) => items.filter((item) => !selectedIds.has(item.id)));
      setSelectedIds(new Set());
      setSuccess(`تم حذف ${selectedIds.size} عرض.`);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'تعذر حذف بعض العروض المحددة.');
    } finally {
    ...
    >
      ...
      <div className="overflow-x-auto rounded-[24px] border border-[#ead9b3] bg-white p-3">
        <table className="min-w-full text-right text-sm text-[#2f2417]">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={8} className="px-3 py-6 text-center text-[#6f6044]">لا توجد عروض تطابق هذه المعايير بعد.</td></tr>
            ) : (

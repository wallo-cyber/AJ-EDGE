...
export default function CompaniesPage(){
 ...
 useEffect(()=>{
   ...
   if (requestedView && ['all','a','missing-dm','ready','followup','opportunity'].includes(requestedView)) {
 ...
 useEffect(()=>{
   ...
   if (view !== 'all') params.set('view', view); else params.delete('view');
   if (segment) params.set('segment', segment); else params.delete('segment');
   if (query.trim()) params.set('q', query.trim()); else params.delete('q');
   const next = params.toString();
   const target = next ? `?${next}` : '';
   if (window.location.pathname + window.location.search !== `/companies${target}`) {
     router.replace(`/companies${target}`, { scroll: false });
   }
 },[query,segment,view,router,searchParams]);
 useEffect(()=>{
   if(!companies.length)return;
   const editId = searchParams.get('edit');
   const target = editId ? companies.find(c=>c.id===editId) ?? null : null;
   if (target) {
     setEditing(target);
     setFormOpen(true);
     return;
 ...
 return <CRMPage title="الشركات" description="قائمة عمل مختصرة تقود من الشركة إلى الشخص الصحيح ثم الإجراء التالي." action={<button onClick={()=>{setEditing(null);setFormOpen(true);const params=new URLSearchParams(window.location.search);params.delete('edit');const target=params.toString()?`/companies?${params.toString()}`:'/companies';router.replace(target,{scroll:false});}} className="btn-primary">إضافة شركة</button>}>
  ...
  {formOpen&&<CompanyForm initialCompany={editing??undefined} submitLabel={editing?'حفظ التعديلات':'إضافة الشركة'} onSubmit={save} onCancel={closeForm}/>} 
  {loading?<div className="crm-empty animate-pulse">جارٍ تحميل الشركات…</div>:<>
   ...
   <div className="mobile-cards grid gap-3">{visible.map(c=><article key={c.id} className="crm-card p-4"><Link href={`/companies/${c.id}`} className="flex items-start justify-between"><div><strong>{c.companyName}</strong><p className="text-xs text-[#75664d]">{c.sector||'قطاع غير محدد'} · {c.city||'مدينة غير محددة'}</p></div><span className="crm-chip status-warning">أولوية {c.priority||'C'}</span></Link><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><p>Lead Score: <b>{c.leadScore??0}</b></p><p>الاكتمال: <b>{c.dataCompleteness??0}%</b></p></div><p className="mt-3 text-sm"><b>التواصل:</b> {stateLabel[states.get(c.id)??'CONTACT_NEEDED']}</p><p className="text-sm"><b>التالي:</b> {c.nextAction||'استكمال الملف'}</p><div className="mt-3"><CompanyActions companyId={c.id} archived={Boolean(c.archivedAt)} onEdit={()=>{setEditing(c);setFormOpen(true);const params=new URLSearchParams(window.location.search);params.set('edit',c.id);router.replace(`/companies?${params.toString()}`,{scroll:false});}} onArchive={()=>void archive(c)}/></div></article>)}</div>

...
export function DailyWorkspace() {
  ...
  const load = () => Promise.all(['companies', 'company_discovery', 'contacts', 'follow_ups', 'opportunities', 'messages', 'user_settings'].map(async (table) => [table, await simpleCrud.list(table)] as const)).then((rows) => setData(Object.fromEntries(rows))).catch((reason: Error) => setError(reason.message)).finally(() => setLoading(false));
  useEffect(() => { void load(); }, []);

...
export function DailyWorkspace() {
  ...
  const actions = useMemo(() => {
    ...
    jobs.filter(j=>j.status==='manual_research_required'&&company(j.company_id)?.priority!=='A').forEach(j=>{const c=company(j.company_id);list.push({id:`j-${j.id}`,rank:2,company:c,label:safe(c?.company_name)||safe(j.agent_name),reason:'قرار أو دليل ناقص يمنع تقدم الشركة',action:safe(j.last_error)||'إضافة دليل موثوق أو سبب تعذر التحقق',href:`/research?tab=manual&job_id=${j.id}`,last:safe(j.updated_at)})});
    ...
    ordered.forEach(item=>{const key=`${item.rank}-${safe(item.company?.id)||item.id}`;if(!unique.has(key))unique.set(key,item)});

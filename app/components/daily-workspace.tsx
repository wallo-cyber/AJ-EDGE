...
export function DailyWorkspace() {
  ...
  return <CRMPage title="مركز العمل اليومي" description={`صندوق مهام تنفيذي بحد ${outreachLimit} تواصل جديد و${followUpLimit} متابعة، مرتب حسب الأولوية.`}>{error && <p className="rounded-xl bg-red-50 p-3 text-red-700">{error}</p>}{notice && <p className="rounded-xl bg-emerald-50 p-3 text-emerald-700">{notice}</p>}{loading ? <div className="crm-empty animate-pulse">جارٍ تجهيز يومك...</div> : <><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">{cards.map(([label, count], index) => <div key={String(label)} className="crm-kpi"><div className="flex items-start justify-between"><p className="text-xs font-semibold text-[#75664d]">{label}</p><span className="text-[#b38b3c]">{['◷','⌁','!','●','A','→'][index] ?? '•'}</span></div><strong className="mt-3 block text-3xl">{count}</strong></div>)}</div><div className="crm-card flex flex-wrap items-center justify-between gap-3 p-4"><div><h3 className="font-bold">صندوق مهام اليوم</h3><p className="text-xs text-[#75664d]">ابدأ بأعلى أولوية ثم انتقل تلقائياً للمهمة التالية.</p></div><button onClick={() => setStarted(true)} className="rounded-xl bg-[#2f2417] px-6 py-3 text-sm font-bold text-white shadow-lg hover:bg-[#49351f]">ابدأ يومي · {tasks.length}</button></div>{started && <section className="crm-card overflow-hidden"><div className="flex items-center justify-between border-b border-[#eadfc9] bg-[#faf5eb] p-4"><h3 className="font-bold">قائمة العمل</h3><span className="crm-chip bg-[#2f2417] text-white">{tasks.length} مهمة</span></div><div className="divide-y divide-[#eee3cd]">{tasks.map((task, index) => { const site = external(task.company?.website); const linkedIn = external(task.company?.linkedin_company || task.company?.linked_in || task.company?.linkedin); const phone = safe(task.company?.general_phone || task.company?.phone); return <article key={task.id} className="group flex gap-3 p-4 hover:bg-[#fdf9f1]"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#f0e3ca] text-sm font-bold text-[#6d5125]">{index + 1}</div><div className="min-w-0 flex-1"><Link href={task.href} className="font-semibold hover:text-[#9a742b]">{task.label}</Link><div className="mt-3 flex flex-wrap gap-2 text-xs">{task.company && <Link href={`/companies/${task.company.id}`} className="rounded-lg border px-3 py-1.5">فتح الشركة</Link>}{task.draft && <button onClick={() => void copyDraft(task)} className="rounded-lg border px-3 py-1.5">نسخ المسودة</button>}{site && <a href={site} target="_blank" rel="noreferrer" className="rounded-lg border px-3 py-1.5">الموقع</a>}{linkedIn && <a href={linkedIn} target="_blank" rel="noreferrer" className="rounded-lg border px-3 py-1.5">LinkedIn</a>}{phone && <a href={`tel:${phone}`} className="rounded-lg border px-3 py-1.5">اتصال</a>}{task.followUp && <button onClick={() => void complete(task)} className="rounded-lg bg-emerald-700 px-3 py-1.5 text-white">إكمال</button>}<Link href="/opportunities" className="rounded-lg border px-3 py-1.5">إنشاء فرصة</Link><Link href="/follow-ups" className="rounded-lg border px-3 py-1.5">جدولة متابعة</Link><button onClick={() => setSkipped((items) => [...items, task.id])} className="rounded-lg border px-3 py-1.5 text-[#75664d]">تخطي</button></div></div></article>; })}{!tasks.length && <div className="crm-empty m-4">لا توجد مهام مستحقة.</div>}</div></section>}</>}</CRMPage>;
}

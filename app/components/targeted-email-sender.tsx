...
const templateFor = (segment: string, companyName: string) => {
  ...
  return {
    ...
    body: `السادة/ ${companyName}\n\nتحية طيبة،\n\nنتواصل معكم من نوفافيرك للتعارف وبحث ${angle}. يسعدنا مشاركة ملفنا التعريفي والتعرف على آلية التسجيل أو التأهيل والفرص المناسبة لديكم.\n\nمع خالص التحية،\nفريق تطوير الأعمال - نوفافيرك`,
  };
};
...
export function TargetedEmailSender() {
  ...
  return <section className="crm-card p-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold text-[var(--nav-accent)]">إرسال موجه</p><h2 className="text-xl font-bold">إرسال بريد لشركة أو قطاع</h2><p className="mt-1 text-sm text-[#8f96a3]">اختر القطاع، راجع مسودة كل شركة وبريدها، ثم أرسل يدويًا.</p></div><button onClick={prepare} className="btn-secondary">تجهيز المعاينة</button></div>

...
import { DecisionAccessWorkspace } from './decision-access-workspace';
...
type CompanyTab = 'overview' | 'access' | 'intelligence' | 'contacts' | 'research' | 'outreach' | 'relationship' | 'pipeline' | 'activity' | 'decisionMakers' | 'vendorRegistration' | 'followups' | 'meetings' | 'opportunities';
const companyTabs: { id: CompanyTab; label: string }[] = [
  { id: 'overview', label: 'نظرة عامة' }, { id: 'access', label: 'الوصول وصنّاع القرار' }, { id: 'intelligence', label: 'الذكاء' }, { id: 'contacts', label: 'جهات الاتصال' },
  { id: 'research', label: 'البحث' }, { id: 'outreach', label: 'التواصل' }, { id: 'relationship', label: 'العلاقة' },
...
export function CompanyDetailsView({ company }: CompanyDetailsViewProps) {
  ...
  const [marketReadiness,setMarketReadiness]=useState<{items:SimpleRow[];gateways:SimpleRow[];company:SimpleRow|null}>({items:[],gateways:[],company:null});

  useEffect(() => {
    void Promise.all([simpleCrud.forCompany('contacts',company.id),simpleCrud.forCompany('follow_ups',company.id),simpleCrud.forCompany('messages',company.id),simpleCrud.forCompany('meetings',company.id),simpleCrud.forCompany('opportunities',company.id),simpleCrud.forCompany('audit_events',company.id),simpleCrud.forCompany('agent_jobs',company.id),simpleCrud.forCompany('communication_events',company.id),simpleCrud.forCompany('business_signals',company.id).catch(()=>[])]).then(([contactRows, followUpRows, messages, meetings, opportunities, auditEvents, agentJobs, communicationEvents, businessSignals]) => {
  ...
  useEffect(()=>{void Promise.all([
    ...
    simpleCrud.page('companies',1,1,{column:'id',value:company.id,order:'created_at'})
  ]).then(([a,b,c])=>setMarketReadiness({items:a.rows,gateways:b.rows,company:c.rows[0]??null})).catch(()=>{})},[company.id]);

  const eligibility=useMemo(()=>commercialEligibility(
    (marketReadiness.company
      ? {
          relationship_type: marketReadiness.company.relationship_type,
          required_gateway_key: marketReadiness.company.required_gateway_key,
          company_type: marketReadiness.company.company_type,
        }
      : { company_type: company.companyType }),
    marketReadiness.items as unknown as ReadinessItem[],
    marketReadiness.gateways as unknown as Gateway[]
  ),[marketReadiness,company.companyType]);
  const missingLabels=eligibility.missing.map(k=>SEED_ITEMS.find(i=>i.item_key===k)?.label||k);
  ...
  return (
    <div className="space-y-4">
      ...
      {activeTab === 'access' ? (
        companyRow ? <DecisionAccessWorkspace company={companyRow} contacts={operational.contacts ?? []} signals={operational.signals ?? []} events={operational.communication_events ?? []} meetings={operational.meetings ?? []} opportunities={operational.opportunities ?? []}/> : <div className="crm-empty">تعذر تجهيز خريطة الوصول لهذه الشركة.</div>
      ...
      <section className="rounded-[24px] border border-[#ead9b3] bg-white p-5">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href={`/contacts?company_id=${company.id}`} className="btn-ghost !rounded-full !px-3 !py-1.5 !text-xs">إضافة جهة اتصال لهذه الشركة</Link>
            <button onClick={() => setIsFollowUpOpen(true)} className="btn-ghost !rounded-full !px-3 !py-1.5 !text-xs">إضافة متابعة</button>
            <Link href="/ready-outreach" className="btn-secondary !rounded-full !px-3 !py-1.5 !text-xs">مراجعة المسودات</Link>
          </div>
        </div>
        <div className="mt-4 grid gap-4 md:grid-cols-2">

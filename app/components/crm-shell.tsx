...
const secondaryGroups = [
  ...
  { label: 'العلاقات والتواصل', links: [['/vendors', 'الموردون'], ['/research', 'البحث والتحقق']] },
...
export function CRMPage({ title, description, action, children }: Props) {
  ...
  return <div className="min-h-screen text-[#2f2417]">
    <header className="نوفافيرك-mobile-header sticky top-0 z-40 flex items-center justify-between border-b border-[#e7d8b8] bg-[#fffdf9]/95 px-4 py-3 backdrop-blur lg:hidden">
      <div><strong>نوفافيرك</strong><span className="mr-2 text-xs text-[#8a6f35]">تطوير الأعمال</span></div>
    ...
    <div className="mx-auto flex max-w-[1680px] gap-4 px-3 py-4 sm:px-5 lg:px-6 lg:py-5">
      ...
      <aside className={`نوفافيرك-sidebar ${open ? 'translate-x-0' : 'translate-x-full'} ${collapsed ? 'lg:w-[78px]' : 'lg:w-60'} fixed inset-y-0 right-0 z-50 w-[286px] overflow-y-auto border-l border-[#e8d9b7] bg-[#fffdf9] p-3 shadow-2xl transition-[width,transform] lg:sticky lg:top-5 lg:z-auto lg:h-[calc(100vh-2.5rem)] lg:translate-x-0 lg:rounded-3xl lg:border` }>
        ...
        <Link href="/" onClick={() => setOpen(false)} className="نوفافيرك-brand mb-4 block rounded-2xl bg-[#2f2417] p-4 text-[#fff8e8]" aria-label="العودة إلى واجهة الموقع">
          <p className="text-[11px] font-bold tracking-[.3em] text-[#d5b66f]">نوفافيرك</p>
      ...
      <main className="نوفافيرك-main min-w-0 flex-1 rounded-3xl border border-[#e8d9b7] bg-[#fffdf9]/92 p-4 shadow-[0_16px_45px_rgba(70,48,18,.06)] sm:p-5 lg:p-6">
        <div className="mb-5 flex flex-col gap-3 border-b border-[var(--nav-border)] pb-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-bold text-[var(--nav-secondary)]">نوفافيرك · تطوير الأعمال</p><span className="nav-current-chip">القسم الحالي: {currentNavLabel}</span></div><h2 className="mt-1 text-2xl font-bold sm:text-3xl">{title}</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-[#75664d]">{description}</p></div>{action}</div>

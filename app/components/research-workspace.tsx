  ...
  return (
    ...
    >
      <section className="crm-card p-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold text-[#ff9d5c]">التجديد الدوري</p><h2 className="text-lg font-bold">تحديث البحث والتحقق</h2><p className="mt-1 text-sm text-[#8f96a3]">يظهر تنبيه عند حلول الموعد، ولا تُزال النتائج القديمة إلا بعد سؤالك.</p></div><div className="flex flex-wrap items-center gap-2"><select value={refreshDays} onChange={(event)=>{const days=Number(event.target.value);setRefreshDays(days);localStorage.setItem("research-refresh-days",String(days))}} className="rounded-xl border p-2"><option value={7}>كل 7 أيام</option><option value={30}>كل 30 يومًا</option><option value={90}>كل 90 يومًا</option></select><span className="crm-chip status-warning">{staleJobs.length} قديمة</span><button disabled={refreshing} onClick={()=>void refreshResearch()} className="btn-primary">{refreshing?"جارٍ التجديد…":"تجديد البحث الآن"}</button></div></div>
      </section>

...
export function ReadyOutreachWorkspace() {
  ...
  const load = () => Promise.all([simpleCrud.list('companies'), simpleCrud.list('messages'), simpleCrud.list('contacts')]).then(([a, b, c]) => { setCompanies(a); setMessages(b); setContacts(c); }).catch((reason: Error) => setError(reason.message)).finally(() => setLoading(false));
  useEffect(() => { void load(); }, []);
  ...
  const markContacted = async (item: (typeof rows)[number], draft: SimpleRow) => {
    ...
    const noResponse = outcome === 'No Response', afterSecondFollowUp = noResponse && safe(draft.template_name) === 'Follow-up 2';
    const stopped = outcome === 'Not Interested' || afterSecondFollowUp;
    const followUpDays = noResponse && safe(draft.template_name) === 'Initial Email' ? 3 : 5;
    const history = Array.isArray(item.company.communication_history) ? item.company.communication_history : [];
    await simpleCrud.update('companies', item.company.id, { last_contact: now.slice(0, 10), outreach_status: afterSecondFollowUp ? 'Nurture' : stopped ? 'Stopped' : 'Contacted', last_outcome: outcome, next_follow_up: stopped ? null : addBusinessDays(followUpDays), nurture_until: stopped ? addBusinessDays(90) : null, communication_history: [...history, { id: crypto.randomUUID(), date: now.slice(0, 10), type: safe(draft.channel), content: `Contacted — ${outcome}`, contact_id: item.contact?.id ?? null }] });
    if (!afterSecondFollowUp && ['No Response', 'Follow-up Later', 'Requested Company Profile', 'Requested More Information', 'Requested Call', 'Requested Vendor Registration'].includes(outcome)) await simpleCrud.create('follow_ups', { company_id: item.company.id, company_name: safe(item.company.company_name), contact_id: item.contact?.id ?? null, contact_person: safe(item.contact?.full_name || item.contact?.name), follow_up_type: outcome === 'Requested Call' ? 'Call' : 'Email', date: addBusinessDays(followUpDays), time: '09:00', priority: safe(item.company.priority) === 'A' ? 'High' : 'Medium', status: 'Pending', subject: outcome === 'Requested Vendor Registration' ? 'التسجيل كمورد/مقاول' : safe(draft.template_name) === 'Initial Email' ? 'Follow-up 1' : 'Follow-up 2', next_action: outcome === 'Requested Vendor Registration' ? 'فتح رابط التسجيل واستكمال المتطلبات' : 'Follow-up', outcome });
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'تعذر تسجيل التواصل.'); }
  };
  ...
  return <CRMPage title="جاهز للتواصل" description="مراجعة واعتماد ونسخ يدوي فقط؛ لا يوجد إرسال جماعي أو تلقائي.">
    ...
    {loading ? <p className="p-8 text-center">جارٍ التحميل...</p> : <div className="grid gap-3">{rows.slice(0, 30).map((item) => { const draft = item.drafts.find((value) => value.template_name === 'Initial Email') ?? item.drafts[0]; const emailDraft = item.drafts.find((value) => value.template_name === 'Initial Email'); const whatsAppDraft = item.drafts.find((value) => value.template_name === 'Short WhatsApp'); const linkedIn = safe(item.contact?.linked_in || item.contact?.linkedin || item.company.linkedin_company || item.company.linked_in || item.company.linkedin); const site = websiteUrl(item.company.website); const phone = safe(item.contact?.mobile || item.contact?.phone || item.company.general_phone || item.company.phone); return <article key={item.company.id} className="rounded-2xl border bg-white p-4"><div className="flex flex-wrap justify-between gap-2"><div><h3 className="font-semibold">{safe(item.company.company_name)}</h3><p className="text-xs">{safe(item.company.company_type)} · Priority {safe(item.company.priority)} · {safe(item.company.lead_score)}/100 · {safe(item.contact?.full_name || item.contact?.name || 'Decision Maker Needed')}</p><p className="text-xs text-[#6f6044]">{safe(item.contact?.position || item.contact?.decision_role)} · {safe(draft.channel)} · Vendor: {safe(item.company.vendor_registration_status || 'Not Checked')} · آخر تواصل: {safe(item.company.last_contact) || '—'} · المتابعة: {safe(item.company.next_follow_up) || '—'}</p></div><span className="rounded-full bg-[#f8efe0] px-3 py-1 text-xs">{safe(draft.status)}</span></div><textarea defaultValue={safe(draft.body)} onBlur={(event) => void simpleCrud.update('messages', draft.id, { body: event.target.value })} className="mt-3 min-h-32 w-full rounded-xl border p-3 text-sm"/><div className="mt-2 flex flex-wrap gap-2 text-xs">{emailDraft && <button onClick={() => void navigator.clipboard.writeText(safe(emailDraft.body)).then(() => setNotice('تم نسخ البريد.'))} className="rounded-full border px-3 py-2">نسخ البريد</button>}{whatsAppDraft && <button onClick={() => void navigator.clipboard.writeText(safe(whatsAppDraft.body)).then(() => setNotice('تم نسخ WhatsApp.'))} className="rounded-full border px-3 py-2">نسخ WhatsApp</button>}{site && <a href={site} target="_blank" rel="noreferrer" className="rounded-full border px-3 py-2">الموقع</a>}{linkedIn && <a href={websiteUrl(linkedIn)} target="_blank" rel="noreferrer" className="rounded-full border px-3 py-2">LinkedIn</a>}{phone && <a href={`tel:${phone}`} className="rounded-full border px-3 py-2">اتصال</a>}<Link href={`/companies/${item.company.id}`} className="rounded-full border px-3 py-2">فتح الشركة</Link><button onClick={() => void simpleCrud.update('messages', draft.id, { status: 'Approved', approved_at: new Date().toISOString() }).then(load)} className="rounded-full border px-3 py-2">اعتماد المسودة</button><button onClick={() => void markContacted(item, draft)} className="rounded-full bg-[#2f2417] px-3 py-2 text-white">Mark Contacted</button><button onClick={() => void simpleCrud.update('messages', draft.id, { status: 'Skipped' }).then(load)} className="rounded-full border px-3 py-2">تخطي اليوم</button><Link href="/enrichment" className="rounded-full border px-3 py-2">بحث إضافي</Link></div></article>; })}{rows.length === 0 && <p className="p-8 text-center">لا توجد شركات جاهزة حالياً.</p>}</div>}
  </CRMPage>;
}

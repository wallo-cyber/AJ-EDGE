...
import { dataCompleteness, duplicateStatus, safeText, scoreDetails, validateImportRows, type DiscoveryInput, type DiscoveryStatus } from './core';
...
const toRow = (item: DiscoveryInput, status: DiscoveryStatus = 'جديد') => { const scoring=scoreDetails(item); return ({ company_name: safeText(item.companyName), company_type: safeText(item.companyType), sector: safeText(item.sector), activity:safeText(item.activity), address:safeText(item.address), tags:safeText(item.tags).split(',').map(v=>v.trim()).filter(Boolean), city: safeText(item.city), website: safeText(item.website), general_phone: safeText(item.generalPhone), general_email: safeText(item.generalEmail), contact_name: safeText(item.contactName), contact_position: safeText(item.contactPosition), contact_email:safeText(item.contactEmail),contact_phone:safeText(item.contactPhone), linkedin: safeText(item.linkedIn), discovery_source: safeText(item.discoverySource) || 'CSV', source_url: safeText(item.sourceUrl), verification_status: safeText(item.verificationStatus) || 'بحاجة تحقق', lead_score: scoring.score, score_reasons:scoring.reasons, data_completeness:dataCompleteness(item), duplicate_status:'New Lead', project_signal: Boolean(item.projectSignal), notes: safeText(item.notes), review_status: status, updated_at: new Date().toISOString() }); };
...
export const discoveryService = {
  ...
  async preview(items: DiscoveryInput[]) {
    ...
    const existing:DiscoveryInput[]=[...queued, ...companyInputs]; const {valid,rejections}=validateImportRows(items); const accepted:DiscoveryInput[]=[];let duplicates=0,possibleDuplicates=0,needsReview=0;
    for(const item of valid){const statuses=[...existing,...accepted].map(candidate=>duplicateStatus(item,candidate));if(statuses.includes('Exact Duplicate')){duplicates+=1;continue;}if(statuses.includes('Possible Duplicate'))possibleDuplicates+=1;if(!safeText(item.website)&&!safeText(item.generalPhone)&&!safeText(item.generalEmail))needsReview+=1;accepted.push(item);}
    return {total:items.length,accepted,duplicates,possibleDuplicates,needsReview,rejected:rejections.length,rejections,existing};
  },
  async import(items: DiscoveryInput[]) {
    const report=await this.preview(items); if(!report.accepted.length)return {...report,imported:0};
    const { error } = await getSupabaseClient().from('company_discovery').insert(report.accepted.map((item) => ({...toRow(item, !safeText(item.website) && !safeText(item.generalPhone) && !safeText(item.generalEmail) ? 'بحاجة تحقق' : 'جديد'),duplicate_status:report.existing.some(candidate=>duplicateStatus(item,candidate)==='Possible Duplicate')?'Possible Duplicate':'New Lead'})));

...
function companyFromRow(row: DbRow): Company {
  return {
    ...
    targetSegment:text(row.target_segment), subsector:text(row.subsector), businessAngle:text(row.business_angle), recommendedRole:text(row.recommended_role), recommendedLanguage:text(row.recommended_language), recommendedChannel:text(row.recommended_channel), recommendedMessageStyle:text(row.recommended_message_style), relationshipStage:text(row.relationship_stage), opportunitySignalScore:Number(row.opportunity_signal_score||0), opportunitySignalReason:text(row.opportunity_signal_reason), nextBestActionCode:text(row.next_best_action_code), nextBestActionReason:text(row.next_best_action_reason), nextBestActionConfidence:Number(row.next_best_action_confidence||0), nextBestActionDueAt:text(row.next_best_action_due_at), doNotContact:Boolean(row.do_not_contact), outreachCooldownUntil:text(row.outreach_cooldown_until), humanOverride:Boolean(row.human_override),
    qualificationStatus:text(row.qualification_status), qualificationReason:text(row.qualification_reason), contractingAngle:text(row.contracting_angle), nextAction:text(row.next_action), vendorRegistrationUrl:text(row.vendor_registration_url), vendorRegistrationStatus:text(row.vendor_registration_status), vendorRegistrationRequirements:text(row.vendor_registration_requirements), vendorRegistrationAccountStatus:text(row.vendor_registration_account_status), vendorRegistrationLastChecked:text(row.vendor_registration_last_checked), vendorRegistrationNextAction:text(row.vendor_registration_next_action), vendorRegistrationNotes:text(row.vendor_registration_notes), outreachStatus:text(row.outreach_status), verificationStatus:text(row.verification_status), archivedAt:text(row.archived_at), lastResearchedAt:text(row.last_researched_at), communicationHistory: jsonArray(row.communication_history),

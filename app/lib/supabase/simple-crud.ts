...
export const simpleCrud = {
  async page(table: string, page = 1, pageSize = 25, options?: { column?: string; value?: string; order?: string; ascending?: boolean }) {
    ...
    if (!client) return { rows: [] as SimpleRow[], count: 0, page, pageSize };
  ...
  async forCompany(table: string, companyId: string, pageSize = 100) {
    const client = await getAuthenticatedSupabaseClient();
    if (!client) return [] as SimpleRow[];
    const { data, error } = await client.from(table).select('*').eq('company_id', companyId).order('created_at', { ascending: false }).limit(pageSize);
  ...
  async listWhere(table: string, column: string, value: string) {
    ...
    const rows: SimpleRow[] = [];
    for (let from = 0; ; from += pageSize) {
      const { data, error } = await client.from(table).select('*').eq(column, value).order('created_at', { ascending: false }).range(from, from + pageSize - 1);
  ...
  async update(table: string, id: string, values: Record<string, unknown>) {
    ...
    const { data, error } = await client.from(table).update({ ...values, updated_at: new Date().toISOString() }).eq('id', id).select('*').single();
